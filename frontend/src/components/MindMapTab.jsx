...
const MindMapTab = () => {
  ...
  return (
    <div className="mindmap-container animate-fade-in" style={{ padding: '2rem', height: '100%', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      {appState === 'idle' && (
        <div className="upload-container glass-panel" style={{ textAlign: 'center', padding: '4rem 2rem', borderRadius: '1rem', border: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
          <Upload size={48} color="var(--color-text)" style={{ marginBottom: '1rem' }} />
          <h2>Upload Patent Data for Mind Map</h2>
          <p>Please upload an Excel or PDF containing patent information.</p>
      ...
      {appState === 'tree' && treeData && (
         <div className="mindmap-workspace" style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: '1rem', height: '100%' }}>
